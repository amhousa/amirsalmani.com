...
export default function Portfolio() {
  ...
  const handleInfoToggle = (show: boolean) => {
    if (isTouchDevice) {
      setShowInfo(show)
      return
    }
      ...
      <div className="relative min-w-full h-full flex flex-col">
        ...
        <div className="relative flex-1 flex items-center justify-center p-4">
          <div className="relative w-full max-w-5xl aspect-[16/10]">
            {/* MacBook frame */}
            <Image src="/images/macbook-frame.png" alt="MacBook frame" fill className="object-contain" priority />
            {/* Project screenshot */}
        ...
        <button
          ...
          onClick={() => isTouchDevice && handleInfoToggle(!showInfo)}
          onMouseEnter={() => !isTouchDevice && handleInfoToggle(true)}
          onMouseLeave={() => !isTouchDevice && handleInfoToggle(false)}
        >
        ...
        <AnimatePresence>
          {showInfo && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="fixed bottom-20 left-4 right-4 md:left-6 md:right-auto md:top-20 md:bottom-auto 
                        md:w-full md:max-w-md z-30"
              onMouseEnter={() => !isTouchDevice && handleInfoToggle(true)}
              onMouseLeave={() => !isTouchDevice && handleInfoToggle(false)}
            >
              <div className="bg-black/80 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/10 max-h-[calc(100vh-160px)] overflow-y-auto">
                <div className="relative">
                  <h2 className="text-lg md:text-2xl font-bold mb-2 md:mb-4 text-white">{project.title}</h2>
        ...
        <AnimatePresence>
          {showInfo && (
            <motion.div
              ...
              onClick={() => isTouchDevice && handleInfoToggle(false)}
            />
          )}
        </AnimatePresence>
